...
const handleScrape = async () => {
  ...
  try {
    ...
    const scrapedData = await new Promise((resolve, reject) => {
      chrome.tabs.sendMessage(tab.id, { action: "scrape", type: scrapeType }, (response) => {
        if (chrome.runtime.lastError) {
          ...
          return reject(new Error("Failed to communicate with content script."));
        }
        resolve(response);
      });
    });
    ...
    console.log("Scraped Data (Before Processing):", scrapedData);

    // Convert list fields to strings before sending
    Object.keys(scrapedData).forEach(key => {
      if (Array.isArray(scrapedData[key])) {
        scrapedData[key] = scrapedData[key].join(", ");
      }
    });

    console.log("Scraped Data (Processed for Backend):", scrapedData);
...
const dataBaseExportToCSV = () => {
 ...
 let count =0;
   ...
   try {
       ...
       const searchBackend = await axios.post(
           "http://127.0.0.1:5000/api/filters",
           searchData,
       ...
       if (searchBackend.status === 201) {
           ...
           alert("Search filter request failed!");
       }
   } catch (error) {
...
const ScrapeNow = async () => {
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      const { jobs, profiles, companies,posts } = response.data;

      setFilterExport({ jobs, profiles, companies,posts });

      alert("Scrape Now is done! Data fetched successfully.");
      setScrapeNowExport(true);
      setAfterSchedule(false);
      setIsSearchFilter(false);
    } else {
      alert("Search filters error! Unexpected response format.");
    }
  } catch (error) {
